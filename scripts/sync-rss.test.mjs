...
// --- report-relative time phrases (Part 3) ---

test('isValidTimeField accepts common Persian report-relative time phrases', () => {
  for (const phrase of [
    'ساعاتی پیش از گزارش',
    'بامداد امروز',
    'امروز صبح',
...
test('validateExtraction normalizes a report-relative time phrase to empty string and passes validation', () => {
  ...
  const ex = validExtraction({
    time: 'ساعاتی پیش از گزارش',
    dateG: '2026-07-08',
    dateP: '۱۷ تیر ۱۴۰۵',
  });
  const { ok, errors } = validateExtraction(ex, LOCS);
  assert.equal(ok, true);
  assert.equal(ex.time, '');
...
// --- weapon/outcome/target "unknown"/"null" normalization (Part 3) ---

test('validateExtraction normalizes literal "unknown"/"null"/"none" weapon/outcome/target values to "نامشخص", case-insensitively', () => {
  const LOCS = { princeHassan: { name: 'x', country: 'jordan' } };
  for (const field of ['weapon', 'outcome', 'target']) {
    for (const raw of ['unknown', 'Unknown', 'NULL', 'none']) {
      const ex = validExtraction({ [field]: raw, dateG: '2026-07-08', dateP: '۱۷ تیر ۱۴۰۵' });
      const { ok, errors } = validateExtraction(ex, LOCS);
      assert.equal(ex[field], 'نامشخص');
      assert.equal(ok, true);
...
test('validateExtraction leaves genuine weapon/outcome/target text untouched', () => {
  const LOCS = { princeHassan: { name: 'x', country: 'jordan' } };
  const ex = validExtraction({
    weapon: 'موشک بالستیک',
    outcome: 'هدف قرار گرفت',
    target: 'پایگاه هوایی',
    dateG: '2026-07-08',
    dateP: '۱۷ تیر ۱۴۰۵',
  });
  const { ok } = validateExtraction(ex, LOCS);
  assert.equal(ok, true);
  assert.equal(ex.weapon, 'موشک بالستیک');
  assert.equal(ex.outcome, 'هدف قرار گرفت');
  assert.equal(ex.target, 'پایگاه هوایی');
});
